...
import { formatToThaiShortDate, formatToThaiFullDate } from '../utils/dateUtils';
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-3xl p-8 max-w-md w-full border border-[#cac3da] shadow-2xl">
            ...
            <form onSubmit={submitNewActivity} className="space-y-4">
              ...
              <div>
                <label className="block text-xs font-black text-[#7a7489] uppercase mb-1">วันที่ดำเนินงาน (เช่น {formatToThaiShortDate()})</label>
                <input
                  type="text"
                  required
                  value={formData.dateStr}
                  onChange={(e) => setFormData({ ...formData, dateStr: e.target.value })}
                  placeholder={`เช่น ${formatToThaiFullDate()}`}
                  className="w-full px-4 py-2.5 rounded-xl border border-[#cac3da] text-sm focus:ring-2 focus:ring-[#0f766e] outline-none"
                />
              </div>
